    ...
    if (isRelativeApi) {
      ...
      try {
        ...
        if (originalResponse.status === 404 || isHtmlResponse) {
          console.warn(`[Fetch Interceptor] Express API returned ${originalResponse.status} ${isHtmlResponse ? '(HTML response detected)' : ''}. Switching to direct client-side Local Fallback Mode in-memory!`);
        ...
        console.warn('[Fetch Interceptor] Network connection failed. Switching to direct client-side Local Fallback Mode in-memory!', networkError);
        ...
        try {
          ...
          return makeMockResponse(500, { success: false, message: 'Fallback to direct client-side Local Fallback failed' });
        }
      }
    }
